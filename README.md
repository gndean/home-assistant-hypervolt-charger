...
| Hypervolt LED Brightness (Light) | Controls the charger LED brightness as a Home Assistant light (on/off + dimming). Turning on without specifying brightness restores the last non-zero brightness (or defaults to 100%). | Preferred. The LED Mode isn't currently supported by the integration. |
| Hypervolt LED Brightness (Number, legacy) | Reads and sets the LED brightness, in percent, as available via Settings within the app. | Deprecated. Disabled by default for new installs; existing installs can keep using it. |
